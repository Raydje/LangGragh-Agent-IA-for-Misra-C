...
# Mock state structure for type checking in the test
...
async def run_live_tests():
    ...
    state_pass: MockComplianceState = {
        "code_snippet": "int divide(int a, int b) { if (b == 0) return 0; return a / b; }",
        "retrieved_rules": [{"rule_id": "MISRA_RULE_7.1"}, {"rule_id": "MISRA_DIR_4.15"}, {"rule_id": "MISRA_RULE_1.3"}, {"rule_id": "MISRA_RULE_8.16"}, {"rule_id": "MISRA_DIR_2.1"} ],
    ...
    print(f"Tokens            : {result_pass.get('total_tokens')} (Cost: ${result_pass.get('estimated_cost', 0):.6f})\n")

    # ---------------------------------------------------------
    # Test Case 2: FAILING CRITIQUE (Rule Hallucination)
    # ---------------------------------------------------------
    print("--- Test Case 2: FAILING CRITIQUE (Hallucination) ---")
    state_hallucination: MockComplianceState = {
    ...
    print(f"Tokens            : {result_fail_hallucination.get('total_tokens')} (Cost: ${result_fail_hallucination.get('estimated_cost', 0):.6f})\n")

    # ---------------------------------------------------------
    # Test Case 3: FAILING CRITIQUE (Logical Inconsistency)
    # ---------------------------------------------------------
    print("--- Test Case 3: FAILING CRITIQUE (Logic Error) ---")
    state_logic_error: MockComplianceState = {
    ...
    print(f"Tokens            : {result_fail_logic.get('total_tokens')} (Cost: ${result_fail_logic.get('estimated_cost', 0):.6f})\n")

    # ---------------------------------------------------------
    # Test Case 4: FAILING CRITIQUE (Missing Rule Category format)
    # ---------------------------------------------------------
    print("--- Test Case 4: FAILING CRITIQUE (Missing Category) ---")
    state_format_error: MockComplianceState = {
    ...
    print(f"Tokens            : {result_fail_format.get('total_tokens')} (Cost: ${result_fail_format.get('estimated_cost', 0):.6f})\n")
